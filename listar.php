...
do{
    if($clientes['backup']!='Sim')
    {
        ...
        foreach($clientes as $column){
            if($count==7){
            $count=0;
            echo "<td>";//apartir daqui começa a imprimir o html responsavel pelo modal?>
            <!-- Botão que inicia o modal -->
            <a href="#myModal<?=$obs?>" role="button" class="btn" data-toggle="modal"><i class='icon-exclamation-sign' title="Observa&ccedil;&atilde;o"></i></a>
            <!-- Modal -->
            ...
            </div><?php//aqui finaliza a impressão do modal
            echo "</td>";
            $obs++;
            }else{
                echo "<td>$column</td>";
